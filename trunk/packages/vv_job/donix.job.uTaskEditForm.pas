...
uses
  uDefines, uFunctions, uStepTypeSelectForm, uDatabasesForm, uStepBasicForm, uStepFactory,
...
begin
  ...
  try
    ...
    begin
      ...
      LStep := TStepFactory.GetStep(StepType, CurrentTask.TaskVar);
      if LStep <> nil then
      begin
        LStepConfigJson := TJSONObject.Create;
        LStepDefine := TStepFactory.GetStepDefine(StepType);
...
begin
  ...
  //根据StepData创建Step
  LForm := TStepFactory.GetStepSettingForm(LStepData.StepType, CurrentTask.TaskVar);
  if LForm <> nil then
  try
